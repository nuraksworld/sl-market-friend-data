...
# Gold (no key): XAU in USD per troy ounce
GOLD_URL = "https://api.gold-api.com/price/XAU"

OUT_PATH = "prices.json"
SCRIPT_VERSION = "fx-gold-api-v1"
...
def fetch_fx():
    ...
    if data.get("result") != "success":
        raise RuntimeError(f"FX API not success: {data}")
    ...
    if not usd_lkr or not usd_gbp or not usd_eur:
        raise RuntimeError("Missing LKR/GBP/EUR in FX response")
...
def fetch_gold_lkr_per_gram(usd_lkr: float):
    data = http_get_json(GOLD_URL)
    usd_per_oz = data.get("price") or data.get("value") or data.get("rate")
    if usd_per_oz is None:
        raise RuntimeError(f"Gold API missing price: {data}")

    usd_per_oz = float(usd_per_oz)
    lkr_per_oz = usd_per_oz * usd_lkr
    lkr_per_g_24k = lkr_per_oz / TROY_OUNCE_TO_GRAM
    lkr_per_g_22k = lkr_per_g_24k * (22.0 / 24.0)

    return {
        "lkr_per_gram_24k": round(lkr_per_g_24k, 2),
        "lkr_per_gram_22k": round(lkr_per_g_22k, 2),
    }
...
def main():
    ...
    try:
        if usd_lkr is None:
            raise RuntimeError("USD->LKR missing; cannot compute gold LKR.")
    ...
    print("Updated", OUT_PATH, payload["lastUpdated"])
